...
const PortfolioModal: React.FC<PortfolioModalProps> = ({ item, onClose }) => {
  ...
  const imagesDark = [item.imageUrl, ...(item.images || [])];
  // If no light images provided, fall back to dark images
  const imagesLight = item.imageUrlLight
    ? [item.imageUrlLight, ...(item.imagesLight || [])]
    : imagesDark;
  ...
  const currentImageDark = imagesDark[activeImageIndex] || imagesDark[0];
  const currentImageLight = imagesLight[activeImageIndex] || imagesLight[0];
      ...
      <div className="relative w-full max-w-4xl bg-white dark:bg-gray-900 rounded-2xl shadow-2xl overflow-hidden flex flex-col max-h-[90vh] animate-in fade-in zoom-in duration-300">
        ...
        <div className="flex flex-col md:flex-row h-full overflow-y-auto md:overflow-hidden">
          ...
          <div className="w-full md:w-1/2 bg-gray-100 dark:bg-gray-800 p-4 flex flex-col">
            <div className="relative aspect-video w-full rounded-lg overflow-hidden mb-4 shadow-md transition-all duration-300">
              <img
                src={currentImageDark}
                alt={item.title}
                className={`w-full h-full object-cover ${
                  currentImageDark !== currentImageLight
                    ? "hidden dark:block"
                    : ""
                }`}
              />
            ...
            {imagesDark.length > 1 && (
              <div className="flex gap-2 overflow-x-auto pb-2 scrollbar-hide">
                {imagesDark.map((imgDark, index) => {
                  const imgLight = imagesLight[index] || imgDark;
                  ...
                  return (
                    <button
                      ...
                      className={`relative w-20 h-14 flex-shrink-0 rounded-md overflow-hidden border-2 transition-all ${
                        isActive
                    ...
                    >
                      <img
                        ...
                        alt={`Thumbnail ${index}`}
                        className={`w-full h-full object-cover ${
                          imgDark !== imgLight ? "hidden dark:block" : ""
                        }`}
                      />
                      {imgDark !== imgLight && (
                        <img
